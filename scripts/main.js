...
//var client = new DeepstreamClient('wss://154.deepstreamhub.com?apiKey=4f3d4540-afe5-429e-ae68-160212e376a7')
var client = new DeepstreamClient('160.1.43.116:6020');
...
function startApp(data){
	...
	var currentUser = client.record.getRecord('user/' + client.getUid());		
	console.log('currentUser is ready');
	console.log(currentUser);
	console.log(currentUser.record.name);
	currentUser.whenReady(currentUser => {
		currentUser.set({
			type: 'a-box',
			attr: {
		...
		subscribeToAvatarChanges(currentUser.record.name);
	});
...
function removeAvatar(id){
   ...
   client.record.getRecord(id).delete();
}
...
function subscribeToAvatarChanges(id){
	var newUser = client.record.getRecord(id);
